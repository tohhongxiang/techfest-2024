...
export default function ChatSection({ id }: { id: string }) {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		setIsLoading(true);
		const receivedMessage = await sendMessage(messageToSendText);
		setIsLoading(false);
		...
		<Stack className="h-full pl-4 pr-0 py-0 flex flex-col">
			...
			<ScrollArea pr="sm" className="h-full">
				<Stack className="flex flex-col gap-y-2" ref={messageListRef}>
					{messages.length === 0 ? (
                        <Text c="dimmed"><i>No messages... Type a message to begin!</i></Text>
                    ) : 
					...
					{isLoading && (
						...
						>
							<Flex align={"center"} gap="sm">
								<Loader size="sm" /> <Text c="dimmed">Loading...</Text>
							</Flex>
						</MessageBubble>
					)}
				</Stack>
